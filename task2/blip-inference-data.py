...
def process_video(index, row, file, model, device, start_index, end_index):
    vid = row["media"]
...
for index, row in tqdm(data.iterrows()):
    ...
    if data.loc(index, "media").str.contains("Photo"):
    ...
    elif data.loc(index, "media").str.contains("Video"):
        video_url, video_captions = process_video(
            index, row, file, blip1_model, device, args.start_index, args.end_index
        )
        data_item["content"] = [video_captions] + data_item.get("content", [])

    twitter_data = company_username_inference(data_item.iloc[index]["username"])

    data_item["content"].append(f"expected likes {data_item.iloc[index]['likes']}")
    data_item["content"].append(f"username {data_item.iloc[index]['username']}")
    data_item["content"].append(
        f"company {data_item.iloc[index]['inferred company']}"
    )
    data_item["content"].append(f"date time {data_item.iloc[index]['date']}")
    data_item["content"].append(f"location {twitter_data['location']}")
    data_item["content"].append(f"description {twitter_data['description']}")
