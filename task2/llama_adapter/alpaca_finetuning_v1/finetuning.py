...
class InstructionDataset(Dataset):
    def __init__(self, data_path, model_path, max_words=30, partition="train"):
        ...
        self.ann = sorted(self.ann, key=lambda x: random.random())
        data_length = len(self.ann)
        if partition == "val":
            self.ann = self.ann[:400]
        if partition == "train":
            self.ann = self.ann[400:60_000]
        # if partition == "train2":
        ...
        # if partition == "train4":
        #     self.ann = self.ann[(data_length - 400) * 3 // 4 + 400:]
...
def main(args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        val_stats = val_one_epoch(
            model, tokenizer, data_loader_val, optimizer, device, epoch, loss_scaler, log_writer=log_writer, args=args
        )
